...
fn round_3_verify(n: u64, round_1_msg: &Round1Message, round_2_msg: &Round2Message, round_3_msg: &Round3Message, transcript: &Transcript, gen: &GenOutput) {
    ...
    let beta = transcript.eta.clone().unwrap();
    ...
    let domain = GeneralEvaluationDomain::<F>::new(n as usize).unwrap();
    assert_eq!(domain.vanishing_polynomial().evaluate(&gamma), Z_H_gamma);

    let Q_b_gamma_check = transcript.q_B.clone().unwrap().evaluate(&gamma);
    let Q_b_gamma = (b_gamma * (round_3_msg.f_commit + beta) - F::one()) / Z_H_gamma;
    // assert_eq!(Q_b_gamma_check, Q_b_gamma);
...
fn round_3(f: &Vec<F>, t: &Vec<F>, transcript: &mut Transcript, gen: &GenOutput) -> Round3Message{
    ...
    let h_evals = 
        domain.elements().enumerate().map(|(i, el)| (b0_evals[i] + eta * f[i] + eta * eta * q_B_evals[i] - v) / (el - eta)).collect::<Vec<_>>();
...
mod tests {
    ...
    fn test_round_3() {
        ...
        let n = f_vec.len() as u64;
        round_3_verify(n, &round_1_msg, &round_2_msg, &round_3_msg, &transcript, &gen);
    }
}